...
function getNextPhonetic(phoneticSet, simpleCap, wordObj, forceSimple) {
	let {numeric, word, opts: {phoneticSimplicity}} = wordObj;

	let deriv = getDerivative(numeric);
	wordObj.numeric = getNumericHash(numeric + word); // ?

	let simple = (numeric + deriv) % phoneticSimplicity > 0;
	let cap = simple || forceSimple ? simpleCap : phoneticSet.length

	let phonetic = phoneticSet[numeric % cap];
